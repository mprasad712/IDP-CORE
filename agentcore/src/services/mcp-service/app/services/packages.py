...
async def sync_packages_to_db() -> None:
    async with session_scope() as session:
        ...
        if not package_table_exists:
            logger.warning("Package table missing; skipping mcp-service package sync")
            return
    ...
    if not lock_pkgs:
        logger.warning("No packages found in %s; skipping mcp-service package sync", uv_lock_path)
        return
