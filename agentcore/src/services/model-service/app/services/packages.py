...
async def sync_packages_to_db() -> None:
    async with session_scope() as session:
        ...
        if not package_table_exists:
            logger.warning("Package table missing; skipping model-service package sync")
            return
    ...
    if not lock_pkgs:
        logger.warning("No packages found in %s; skipping model-service package sync", uv_lock_path)
        return
