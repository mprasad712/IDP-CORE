...
def test_boundaries_from_page_texts_page_one_reset_is_strong():
    # page 3 says "Page 1 of 2" -> a strong reset boundary
    page_texts = {0: "INVOICE\nTotal 100", 1: "continued line items", 2: "DELIVERY NOTE\nPage 1 of 2", 3: "items"}
...
def test_boundaries_from_page_texts_header_only_is_hint_not_split():
    # a long single doc whose page 3 heading reuses "invoice" must NOT be split by the rules alone
    page_texts = {0: "cover letter body text here", 1: "more body text continues", 2: "invoice summary section"}
...
async def test_hybrid_rules_only_strong_split_no_model():
    page_texts = {0: "INVOICE", 1: "x", 2: "PAN CARD\nPage 1", 3: "y"}   # 'Page 1' = strong
...
async def test_hybrid_no_model_header_hint_does_not_false_split():
    page_texts = {0: "cover body", 1: "invoice section body"}   # header hint, NO strong signal, NO model
...
async def test_hybrid_escalates_to_llm_and_returns_types():
    page_texts = {0: "acme invoice body", 1: "medical report body no markers"}
