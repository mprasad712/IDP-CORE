...
class McpAuditLog(SQLModel, table=True):  # type: ignore[call-arg]
    __tablename__ = "mcp_audit_log"

    id: UUID = Field(default_factory=uuid4, primary_key=True)
    mcp_id: UUID | None = Field(default=None, foreign_key="mcp_registry.id", nullable=True, index=True)
    action: str = Field(
        default="unknown",
    ...
    deployment_env: str | None = Field(default=None, sa_column=Column(String(20), nullable=True))
    visibility: str | None = Field(default=None, sa_column=Column(String(20), nullable=True))
    ...
    __table_args__ = (
        Index("ix_mcp_audit_action_created", "action", "created_at"),
        Index("ix_mcp_audit_actor_created", "actor_id", "created_at"),
    )


class McpAuditLogRead(BaseModel):
    id: UUID
