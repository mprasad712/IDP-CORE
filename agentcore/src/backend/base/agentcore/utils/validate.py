...
from agentcore.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
...
def validate_code(code):
    ...
    # Evaluate the function definition with agentcore context
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            code_obj = compile(ast.Module(body=[node], type_ignores=[]), "<string>", "exec")
            try:
                # Create execution context with common agentcore imports
                exec_globals = _create_agentcore_execution_context()
...
def _create_agentcore_execution_context():
    """Create execution context with common agentcore imports."""
    context = {}

    # Import common agentcore types that are used in templates
    try:
    ...
    try:
        from agentcore.io import HandleInput, Output, TabInput
...
def create_class(code, class_name):
    ...
    code = code.replace("from agentcore import CustomComponent", "from agentcore.custom import ExecutableNode")
    ...
    code = code.replace(
        "from agentcore.interface.custom.custom_component import CustomComponent",
        "from agentcore.custom import ExecutableNode",
    )
...
def get_default_imports(code_string):
    ...
    agentcore_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, agentcore_imports)
    agentcore_module = importlib.import_module("agentcore.field_typing")
    default_imports.update({name: getattr(agentcore_module, name) for name in necessary_imports})
