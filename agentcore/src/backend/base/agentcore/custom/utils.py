...
from agentcore.custom.schema import NoDefault
...
from agentcore.template.frontend_node.custom_components import NodeFrontendNode, ExecutableNodeFrontendNode
from agentcore.type_extraction.type_extraction import extract_inner_type
from agentcore.utils import validate
from agentcore.utils.util import get_base_classes
...
def run_build_config(
    ...
    # we have imported it
    # If not, it means the component was loaded through AGENTCORE_COMPONENTS_PATH
...
async def get_single_component_dict(component_type: str, component_name: str, components_paths: list[str]):
    ...
    for base_path in components_paths:
        ...
        if module_path.exists():
            ...
            module_name = f"agentcore.components.{component_type}.{component_name}"
            try:
...
async def load_custom_component(component_name: str, components_paths: list[str]):
    ...
    """
    from agentcore.interface.custom_component import get_custom_component_from_name
                ...
                for category_dir in base_path.iterdir():
                    if category_dir.is_dir():
                        ...
                        if component_file.exists():
                            # Try to import the module
                            module_name = f"agentcore.components.{category_dir.name}.{component_name}"
                            try:
                                import importlib.util
