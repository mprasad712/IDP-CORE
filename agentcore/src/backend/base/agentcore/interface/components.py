...
from agentcore.custom.utils import abuild_custom_components, create_component_template
from agentcore.services.settings.base import BASE_COMPONENTS_PATH
...
async def import_agentcore_components():
    """Asynchronously discovers and loads all built-in Agentcore components with module-level parallelization.

    Scans the `agentcore.components` package and its submodules in parallel, instantiates classes that are subclasses
    ...
    modules_dict = {}
    try:
        import agentcore.components as components_pkg
    except ImportError as e:
        logger.error(f"Failed to import agentcore.components package: {e}", exc_info=True)
...
async def get_and_cache_all_types_dict(
    ...
    Supports both full and partial (lazy) loading. If the cache is empty, loads built-in Agentcore
    ...
    if component_cache.all_types_dict is None:
        ...
        agentcore_components = await import_agentcore_components()
        custom_components_dict = await _determine_loading_strategy(settings_service)

        # merge the dicts
        component_cache.all_types_dict = {
            **agentcore_components["components"],
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from agentcore.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from agentcore.custom.utils import build_custom_components
