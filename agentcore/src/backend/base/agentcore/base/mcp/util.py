"""Minimal MCP utilities retained in the backend.

All MCP session management, STDIO/SSE clients, and tool invocation have been
moved to the MCP microservice (port 8002).  Only the JSON-Schema → Pydantic
helper is kept here because ``mcp_node.py`` uses it to reconstruct
``StructuredTool`` objects from the schemas returned by the microservice.
"""
...
# Number of types in a nullable union (the type itself + null)
