...
from agentcore.base.tools.constants import TOOL_OUTPUT_NAME
from agentcore.io.schema import create_input_schema, create_input_schema_from_dict
...
class ComponentToolkit:
    ...
    def get_tools(
        ...
        if len(tools) == 1 and (tool_name or tool_description):
            ...
            tool.tags = [tool.name]
        elif len(tools) > 1 and (tool_name or tool_description):
            for tool in tools:
