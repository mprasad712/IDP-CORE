...
jest.mock("@/hooks/agents/use-delete-agent", () => ({
  __esModule: true,
  default: () => ({
...
describe("SidebarDraggableComponent", () => {
  ...
  describe("OnDelete Behavior", () => {
    ...
    it("should prioritize onDelete over default agent deletion when onDelete is provided", async () => {
      const mockOnDelete = jest.fn();
      ...
      // Verify that our custom onDelete function was called and deleteAgent was NOT called
      expect(mockOnDelete).toHaveBeenCalledTimes(1);
      expect(mockDeleteAgent).not.toHaveBeenCalled();
    });

    it("should use default agent deletion when onDelete is not provided", async () => {
      ...
      // Verify that the default deleteAgent function was called
      expect(mockDeleteAgent).toHaveBeenCalledTimes(1);
      expect(mockDeleteAgent).toHaveBeenCalledWith({ id: "agent1" }); // Based on our mocked agents
    });

    it("should not call deleteAgent when onDelete is not provided and agent is not found", async () => {
      const propsWithNonExistentAgent = {
        ...defaultProps,
        onDelete: undefined,
        official: false,
      ...
      // Verify that deleteAgent was not called since agent was not found
      expect(mockDeleteAgent).not.toHaveBeenCalled();
    });
  });
});
