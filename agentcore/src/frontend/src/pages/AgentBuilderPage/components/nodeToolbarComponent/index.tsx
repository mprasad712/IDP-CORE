...
const NodeToolbarComponent = memo(
  ({
    ...
    const [agentComponent, setAgentComponent] = useState<AgentType>(
      createAgentComponent(cloneDeep(data), version),
    );
    const updateFreezeStatus = useAgentStore(
    ...
    const getNodePosition = useAgentStore((state) => state.getNodePosition);
    ...
    const isSaved = agents?.some((agent) =>
      Object.values(agent).includes(data.node?.display_name!),
    );
    ...
    const handleungroup = useCallback(() => {
      if (isGroup) {
        ...
        expandGroupNode(
          data.id,
          updateAgentPosition(getNodePosition(data.id), data.node?.agent!),
          data.node!.template,
          setNodes,
          setEdges,
          data.node?.outputs,
    ...
    }, [isSaved, data.id, agentComponent, addAgent]);
    ...
    useShortcuts({
      ...
      downloadFunction: () => downloadNode(agentComponent!),
    ...
    useEffect(() => {
      setAgentComponent(createAgentComponent(cloneDeep(data), version));
    }, [
      data,
      data.node,
      ...
      (event) => {
        ...
        switch (event) {
          ...
          case "update":
            ...
            break;
          case "copy": {
            nodes = useAgentStore.getState().nodes;
