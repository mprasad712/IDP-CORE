...
} from "../types/zustand/agent";
import { buildAgentVerticesWithFallback } from "../utils/buildUtils";
import {
...
import useAgentsManagerStore from "./agentsManagerStore";
...
const useAgentStore = create<AgentStoreType>((set, get) => ({
  ...
  addDataToAgentPool: (data: VertexBuildTypeAPI, nodeId: string) => {
    const newAgentPool = cloneDeep({ ...get().agentPool });
    if (!newAgentPool[nodeId]) newAgentPool[nodeId] = [data];
    else {
      newAgentPool[nodeId].push(data);
    }
    get().setAgentPool(newAgentPool);
  },
  ...
  updateAgentPool: (
    ...
    const newAgentPool = cloneDeep({ ...get().agentPool });
    if (!newAgentPool[nodeId]) {
      return;
    } else {
      let index = newAgentPool[nodeId].length - 1;
      if (buildId) {
        index = newAgentPool[nodeId].findIndex((agent) => agent.id === buildId);
      }
      //check if the data is a agentpool object
      if ((data as VertexBuildTypeAPI).valid !== undefined) {
        newAgentPool[nodeId][index] = data as VertexBuildTypeAPI;
      }
      //update data results
      else {
        newAgentPool[nodeId][index].data.message = data as
          | ChatOutputType
          | ChatInputType;
      }
    }
  ...
  resetAgent: (agent) => {
    ...
    set({
      dismissedNodes: JSON.parse(
        localStorage.getItem(`dismiss_${agent?.id}`) ?? "[]",
      ) as string[],
      dismissedNodesLegacy: JSON.parse(
        localStorage.getItem(`dismiss_legacy_${agent?.id}`) ?? "[]",
      ) as string[],
    });
    unselectAllNodesEdges(nodes, newEdges);
    if (agent?.id) {
      useTweaksStore.getState().initialSetup(nodes, agent?.id);
    }
    set({
      nodes,
      edges: newEdges,
  ...
  setAgentState: (agentState) => {
    const newAgentState =
      typeof agentState === "function" ? agentState(get().agentState) : agentState;

    if (newAgentState !== get().agentState) {
      set(() => ({
  ...
  setNodes: (change) => {
    ...
    get().updateCurrentAgent({ nodes: newChange, edges: newEdges });
    if (get().autoSaveAgent) {
      get().autoSaveAgent!();
    }
  },
  setEdges: (change) => {
    ...
    get().updateCurrentAgent({ edges: newChange });
    if (get().autoSaveAgent) {
      get().autoSaveAgent!();
    }
  },
  setNode: (
    ...
    get().updateCurrentAgent({ nodes: newNodes, edges: newEdges });
    if (get().autoSaveAgent) {
      get().autoSaveAgent!();
    }
  },
  ...
  paste: (selection, position) => {
    if (get().currentAgent?.locked) return;
    // Collect IDs of nodes in the selection
    const selectedNodeIds = new Set(selection.nodes.map((node) => node.id));
    // Find existing edges in the agent that connect nodes within the selection
    ...
    if (
      ...
      useAlertStore.getState().setNoticeData({
        title: "You can only have one Chat Input component in a agent.",
      });
  ...
  buildAgent: async ({
    ...
    set({ agentBuildStatus: {} });
    const currentAgent = useAgentsManagerStore.getState().currentAgent;
    ...
    await buildAgentVerticesWithFallback({
      session,
      input_value,
      files,
      ...
      onBuildComplete: (allNodesValid) => {
        ...
        get().setIsBuilding(false);
        trackAgentBuild(get().currentAgent?.name ?? "Unknown", false, {
      ...
      onBuildError: (title: string, list: string[], elementList) => {
        ...
        useAgentStore.getState().updateBuildStatus(idList, BuildStatus.ERROR);
  ...
  updateBuildStatus: (nodeIdList: string[], status: BuildStatus) => {
    const newAgentBuildStatus = { ...get().agentBuildStatus };
    nodeIdList.forEach((id) => {
      newAgentBuildStatus[id] = {
        status,
      };
      if (status == BuildStatus.BUILT) {
        const timestamp_string = new Date(Date.now()).toLocaleString();
        newAgentBuildStatus[id].timestamp = timestamp_string;
      }
    });
    set({ agentBuildStatus: newAgentBuildStatus });
  },
  revertBuiltStatusFromBuilding: () => {
    const newAgentBuildStatus = { ...get().agentBuildStatus };
    Object.keys(newAgentBuildStatus).forEach((id) => {
      if (newAgentBuildStatus[id].status === BuildStatus.BUILDING) {
        newAgentBuildStatus[id].status = BuildStatus.BUILT;
      }
    });
    set({ agentBuildStatus: newAgentBuildStatus });
  },
  ...
  updateCurrentAgent: ({ nodes, edges }) => {
    set({
      currentAgent: {
        ...get().currentAgent!,
        data: {
          nodes: nodes ?? get().currentAgent?.data?.nodes ?? [],
          edges: edges ?? get().currentAgent?.data?.edges ?? [],
          viewport: get().currentAgent?.data?.viewport ?? {
            x: 0,
            y: 0,
            zoom: 1,
          },
  ...
  addDismissedNodesLegacy: (dismissedNodes: string[]) => {
    ...
    localStorage.setItem(
      `dismiss_legacy_${get().currentAgent?.id}`,
      JSON.stringify(newDismissedNodes),
    );
